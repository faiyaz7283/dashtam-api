...
class SyncTransactionsHandler:
    ...
    async def handle(
        ...
        if connection is None:
            return cast(Result[SyncTransactionsResult, str], Failure(error=SyncTransactionsError.CONNECTION_NOT_FOUND))

        # 2. Verify ownership
        if connection.user_id != command.user_id:
            return cast(Result[SyncTransactionsResult, str], Failure(error=SyncTransactionsError.NOT_OWNED_BY_USER))

        # 3. Verify connection is active
        if not connection.is_connected():
            return cast(Result[SyncTransactionsResult, str], Failure(error=SyncTransactionsError.CONNECTION_NOT_ACTIVE))

        # 4. Get and decrypt credentials
        if connection.credentials is None:
            return cast(Result[SyncTransactionsResult, str], Failure(error=SyncTransactionsError.CREDENTIALS_INVALID))
        ...
        if isinstance(decrypt_result, Failure):
            return cast(Result[SyncTransactionsResult, str], Failure(error=SyncTransactionsError.CREDENTIALS_DECRYPTION_FAILED))
        ...
        if command.account_id:
            ...
            if account is None:
                return cast(Result[SyncTransactionsResult, str], Failure(error=SyncTransactionsError.ACCOUNT_NOT_FOUND))
            if account.connection_id != connection.id:
                return cast(Result[SyncTransactionsResult, str], Failure(error=SyncTransactionsError.ACCOUNT_NOT_OWNED))
            accounts = [account]
        else:
        ...
        if not accounts:
            return cast(Result[SyncTransactionsResult, str], Failure(error=SyncTransactionsError.NO_ACCOUNTS))
