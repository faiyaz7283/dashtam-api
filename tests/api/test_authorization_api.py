...
Architecture:
- Uses TestClient with simulated protected endpoints
- Mocks authentication to inject test users with different roles
- Tests HTTP status codes and error responses

Test Strategy:
    This file tests authorization DEPENDENCIES in isolation using a standalone
    test app. This is a "dependency unit test" pattern that validates the
    authorization dependencies work correctly before they're wired to real
    endpoints.

    Once real API endpoints (e.g., GET /accounts, POST /transactions) are
    implemented and use these authorization dependencies, additional API
    integration tests should be added to test the real app with real
    authorization flows.
...
class TestRequireAnyPermission:
    ...
    def test_allows_access_when_first_permission_matches(
    ...
    def test_allows_access_when_second_permission_matches(
        self, test_app, mock_authorization
    ):
...
class TestRequireAllPermissions:
    ...
    def test_returns_403_when_first_permission_missing(
        self, test_app, mock_authorization
    ):
        ...
        response = client.post("/admin/security")

        assert response.status_code == 403

    def test_returns_403_when_second_permission_missing(
        self, test_app, mock_authorization
    ):
    ...
    def test_allows_access_when_all_permissions_granted(
        self, test_app, mock_authorization
    ):
...
class TestEdgeCases:
    ...
    def test_empty_permission_list_for_any(self, test_app):
        ...
        """
        # Create app with no permissions (variadic empty)
        app = FastAPI()

        @app.get("/empty-any")
        async def empty_any(
            _: Annotated[None, Depends(require_any_permission())],  # No args
        ):
            return {"ok": True}
    ...
    def test_empty_permission_list_for_all(self, test_app):
        ...
        """
        # Create app with no permissions required (variadic empty)
        app = FastAPI()

        @app.get("/empty-all")
        async def empty_all(
            _: Annotated[None, Depends(require_all_permissions())],  # No args
        ):
            return {"ok": True}
